...
import { getUserLogin, removeUserLogin, accessAccount, btnCreateArtist } from "../../redux/actions";
const CreateArtists = () => {
  ...
  const messageArtists = useRef(null);
  ...
  const fetchAPI = async () => {
    ...
    if (name.length >= 2 && nameStatus && thumbnailStatus && checkThumbnail && isImage(checkThumbnail)) {
      try {
        ...
        const createMusic = await axios.post("https://random-musics.herokuapp.com/api/v1/artists", {
          name,
  ...
  const handleCloseModal = () => {
    dispatch(btnCreateArtist(false));
    setName("");
  };
  ...
  const createSuccess = (name) => {
    ...
    messageArtists.current.appendChild(spanClass);
    setTimeout(() => {
      removeSuccess();
    }, 1500);
  };
  const removeSuccess = () => {
    messageArtists.current.removeChild(messageArtists.current.childNodes[2]);
  };
        ...
        <div className="modal-opacity">
          <div className="box-modal" ref={wrapperRef} style={{ height: "350px" }}>
            <div className="modal__header">
              <div className="modal__header--title">Create Artist</div>
            ...
            <div className="modal__body">
              <div className="modal__body--message" ref={messageArtists}>
                <span className="message--error" ref={nameError}>
                  Tên nghệ sĩ phải từ 2 kí tự trở lên
                </span>
              ...
              <div className="input-file">
                <input
                  ...
                  name="file"
                  ref={thumbnailInput}
                  id="fileThumbnailArtists"
                  onChange={(e) => handleChangeThumbnail(e)}
                />
                <label htmlFor="fileThumbnailArtists" className="input-label">
                  <BsCloudUpload />
                </label>
